...
const Navbar = () => {
  ...
  const linkClass = (href) =>
    ...
    `after:absolute after:left-0 after:-bottom-1.5 after:h-px after:bg-gold after:transition-all after:duration-300 ` +
    (active === href
      ? 'text-gold after:w-full'
      : 'text-light/70 hover:text-gold after:w-0 hover:after:w-full');
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled ? 'bg-dark/90 backdrop-blur-md shadow-lg shadow-black/30 border-b border-gold/10' : 'bg-transparent'
      }`}
    >
      <div className="max-w-6xl mx-auto px-6 h-16 md:h-20 flex items-center justify-between">
        <a href="#home" className="flex items-center gap-3">
          <img src="/images/logo.svg" alt="Desu Wannabisaya logo" className="w-9 h-9 md:w-10 md:h-10" />
          <span className="text-gold text-lg md:text-xl font-bold tracking-wider">Desu Wannabisaya</span>
        </a>
