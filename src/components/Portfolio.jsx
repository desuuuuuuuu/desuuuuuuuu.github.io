  ...
  return (
    ...
    >
        ...
        <div className="aspect-video overflow-hidden bg-dark-2">
          {imageFailed ? (
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-dark-2 to-dark-3">
              <span className="text-silver/40 text-4xl font-bold">{project.title.charAt(0)}</span>
            </div>
          ) : (
        ...
        {!imageFailed && project.images.length > 1 && (
          <>
            <div className="absolute inset-0 flex items-center justify-between px-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100 transition-opacity duration-300">
              <button
                onClick={prev}
                className="w-9 h-9 rounded-full bg-dark/80 text-light flex items-center justify-center hover:bg-silver/80 transition-colors text-sm"
              ...
              {project.images.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i === currentImage ? 'bg-silver w-4' : 'bg-light/30 w-1.5 hover:bg-light/50'
                  }`}
      ...
      <div className="p-6 flex flex-col flex-1">
        <h3 className="text-lg font-semibold text-silver mb-2">{project.title}</h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.techStack.map((tech) => (
            <span
              key={tech}
              className="px-2.5 py-1 text-[11px] rounded-full bg-silver/10 text-silver border border-silver/20"
            >
              {tech}
            </span>
          ))}
        ...
        <ul className="space-y-1.5 mt-auto">
          {project.features.slice(0, 3).map((feature, i) => (
            <li key={i} className="text-light/50 text-xs flex items-start gap-2">
              <span className="text-silver flex-shrink-0" aria-hidden="true">&#8226;</span>
              {feature}
            </li>
          ))}
          {hiddenFeatures > 0 && (
            <li className="text-silver/60 text-xs pt-0.5">+ {hiddenFeatures} more features</li>
          )}
        </ul>
      </div>
    </motion.div>
